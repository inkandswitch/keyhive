    ...
    > Keyhive<S, T, P, C, L, R>
{
    ...
    /// Convert a [`StaticEvent`] to an [`Event`].
    ///
    /// This method assumes that all dependencies (delegations, revocations, agents)
    /// are already present in the stores. If any dependency is missing, an error is returned.
    #[instrument(skip_all)]
    ...
    pub async fn receive_delegation(
        ...
        let delegation_payload = self.static_delegation_to_delegation(static_dlg).await?;
    ...
    pub async fn receive_revocation(
        ...
        let revocation_payload = self.static_revocation_to_revocation(static_rev).await?;
    ...
    pub async fn into_archive(&self) -> Archive<T> {
        ...
        {
            let locked_pending_events = self.pending_events.lock().await;
            for event in locked_pending_events.iter() {
                pending_events.push(event.as_ref().clone());
            }
        }
    ...
    pub async fn ingest_archive(
        ...
        let events = archive
            ...
            .collect::<Vec<_>>();
        let still_pending = self.ingest_unsorted_static_events(events).await;
        self.pending_events
            .as_ref()
            .lock()
            .await
            .append(&mut still_pending.clone());
        Ok(still_pending)
    }
...
/// Error type for converting a [`StaticEvent`] to an [`Event`].
