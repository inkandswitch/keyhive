...
> {
    ...
    pub ops: HashMap<Digest<MembershipOperation<S, T, L>>, MembershipOperation<S, T, L>>,

    /// For each agent: the set of membership op digests reachable by that agent.
    pub index: HashMap<Identifier, HashSet<Digest<MembershipOperation<S, T, L>>>>,
}
    ...
    > Keyhive<S, T, P, C, L, R>
{
    ...
    pub async fn membership_ops_for_all_agents(&self) -> AllMembershipOps<S, T, L> {
        let mut all_ops: HashMap<Digest<MembershipOperation<S, T, L>>, MembershipOperation<S, T, L>> =
            HashMap::new();
        let mut membered_digests: HashMap<MemberedId, HashSet<Digest<MembershipOperation<S, T, L>>>> =
            HashMap::new();
        ...
        let mut index: HashMap<Identifier, HashSet<Digest<MembershipOperation<S, T, L>>>> =
            HashMap::new();
                ...
                if let Some(entry) = index.get_mut(&identifier) {
                    for rev in agent_revs {
                        ...
                        if entry.insert(hash) {
                            all_ops
                                .entry(hash)
                                .or_insert_with(|| rev.dupe().into());

                            Self::bfs_extend_from_revocation(
                                rev,
                                &mut all_ops,
                                entry,
                            )
        ...
        AllMembershipOps { ops: all_ops, index }
    }
    ...
    ///
    /// Returns a [`AllReachablePrekeyOps`] containing:
    ...
    pub async fn reachable_prekey_ops_for_all_agents(&self) -> AllReachablePrekeyOps {
        ...
        for group in &groups {
            ...
            group_data.push((group_id, transitive));
        }
        ...
        for doc in &docs {
            ...
            doc_data.push((doc_id, transitive));
        }
        ...
        for (group_id, transitive) in &group_data {
            ...
            for (agent_id, (agent, _access)) in transitive {
                if !key_ops_cache.contains_key(agent_id) {
                    key_ops_cache.insert(*agent_id, agent.key_ops().await);
                }
            }
        }
        ...
        let ops: HashMap<Identifier, Vec<Arc<KeyOp>>> = key_ops_cache
            .iter()
            .map(|(id, ca_map)| (*id, topsort_keys(ca_map)))
            .collect();
        ...
        let mut path = "none";
        {
                ...
                .remove(&IndividualId(subject_id))
            {
                ...
                self.groups
                    .lock()
                    .await
                    .insert(GroupId(subject_id), group);
            }
        }
        if !found {
            path = "new";
    ...
    async fn receive_revocation_deferred(
        ...
        if let Some(group) = self.groups.lock().await.get(&GroupId(id)) {
            ...
            locked_doc.receive_revocation_no_rebuild(revocation.clone()).await?;
        ...
        #[cfg(not(target_arch = "wasm32"))]
        {
            ...
            if total.as_millis() > 50 {
                tracing::warn!(
                    "SLOW REV: total={:?} verify={:?} convert={:?} insert={:?} path={}",
                    total,
                    rt1 - rt0,
                    rt2 - rt1,
                    rt3 - rt2,
    ...
    pub async fn ingest_unsorted_static_events(
        ...
        for event in epoch {
            ...
            #[cfg(not(target_arch = "wasm32"))]
            {
                ...
                if ev_elapsed.as_millis() > 100 {
                    tracing::warn!("SLOW EVENT: type={} elapsed={:?} outcome={}",
                        event_type, ev_elapsed,
                        match &result { Ok(Some(_)) => "ok", Ok(None) => "dedup", Err(_) => "fail" });
                }
            }
            ...
            if (success_count + dedup_count + fail_count) % 500 == 0 {
                tracing::info!("FIRST PASS progress: {} ok, {} dedup, {} fail",
                    success_count, dedup_count, fail_count);
            }
        }
        tracing::info!("FIRST PASS totals: {} ok, {} dedup, {} fail",
            success_count, dedup_count, fail_count);

        // Rebuild all modified groups/docs once
        tracing::info!("REBUILD: {} groups/docs, {} failed events for retry",
            modified_ids.len(), next_epoch.len());
        ...
        loop {
            ...
            tracing::info!("RETRY LOOP iteration {}: processing {} events", retry_iteration, epoch_len);
            ...
            tracing::info!("RETRY LOOP iteration {} done: {} succeeded, {} failed",
                retry_iteration, epoch_len - retry_next.len(), retry_next.len());

            if retry_next.is_empty() {
                tracing::info!("RETRY LOOP: all events succeeded, rebuilding {} groups/docs", all_retry_modified_ids.len());
                self.rebuild_modified(&all_retry_modified_ids).await;
            ...
            if retry_next.len() == epoch_len {
                tracing::info!("RETRY LOOP: stuck at fixed point with {} events, rebuilding {} groups/docs",
                    epoch_len, all_retry_modified_ids.len());
                self.rebuild_modified(&all_retry_modified_ids).await;
                tracing::debug!(
