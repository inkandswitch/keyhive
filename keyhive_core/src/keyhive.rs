    ...
    > Keyhive<F, S, T, P, C, L, R>
{
    ...
    /// Take `to_revoke`'s membership in `resource` away.
    ///
...
mod tests {
    ...
    async fn test_revoke_nested_group_removes_correct_cgka_members() -> TestResult {
        ...
        let (carol_id, _carol_indie) = register_peer(&alice, &carol_kh).await;
        ...
        alice.add_member(dave_id, g2_id, Access::Read, &[]).await?;
        alice.add_member(eve_id, g2_id, Access::Read, &[]).await?;
        alice.add_member(frank_id, g2_id, Access::Read, &[]).await?;

        // Create G1: Carol and G2
        let g1 = alice.generate_group(vec![]).await?;
        let g1_id = g1.lock().await.group_id();
        alice.add_member(carol_id, g1_id, Access::Read, &[]).await?;
        ...
        let update = alice
            .revoke_member(
                g1_id, true, // retain other doc members (Bob, Frank)
                doc_id,
            )
            .await?;
    ...
    async fn test_revoke_from_group_does_not_affect_member_doc_cgka() -> TestResult {
        ...
        let update = alice.revoke_member(g2_id, true, g_id).await?;
    ...
    async fn test_revoke_multipath_keeps_cgka_member() -> TestResult {
        ...
        let g = alice.generate_group(vec![]).await?;
        let g_id = g.lock().await.group_id();
        alice.add_member(bob_id, g_id, Access::Read, &[]).await?;
    ...
    async fn test_revoke_leaves_no_key_with_a_direct_relay_member() -> TestResult {
        ...
        let alice = make_keyhive().await;
        let bob_kh = make_keyhive().await;
        let (bob_id, _) = register_peer(&alice, &bob_kh).await;
        ...
        alice.revoke_member(g_id, true, doc_id).await?;
    ...
    async fn test_indirect_cycle_revoke_removes_access() -> TestResult {
        ...
        alice.add_member(g2_id, g1_id, Access::Read, &[]).await?;
        alice.add_member(g3_id, g2_id, Access::Read, &[]).await?;
        alice.add_member(g1_id, g3_id, Access::Read, &[]).await?;
    ...
    async fn test_direct_cycle_both_in_doc_revoke_one_keeps_other() -> TestResult {
        ...
        let update = alice.revoke_member(g1_id, true, doc_id).await?;
    ...
    async fn test_relay_does_not_get_into_cgka_tree() -> TestResult {
        ...
        // a) relay directly on the document
        {
            ...
            let id = mk_person(&hive).await;
            let doc = hive.generate_doc(vec![], nonempty![[1u8; 32]]).await?;
            let doc_id = DocumentId(doc.lock().await.id());
            hive.add_member(id, doc_id, Access::Relay, &[]).await?;
            assert!(
        ...
        // b) relay in a group, then the group is given edit on the document
        {
            ...
            let group_id = { group.lock().await.group_id() };
            hive.add_member(id, group_id, Access::Relay, &[]).await?;
            let doc = hive.generate_doc(vec![], nonempty![[1u8; 32]]).await?;
            let doc_id = DocumentId(doc.lock().await.id());
            hive.add_member(group_id, doc_id, Access::Edit, &[]).await?;
            assert!(
                !{ doc.lock().await.cgka_members()?.any(|m| m == id) },
        ...
        // c) the same, in the other order
        {
            ...
            let id2 = mk_person(&hive).await;
            hive.add_member(id2, group_id, Access::Relay, &[]).await?;
            assert!(
                !{ doc.lock().await.cgka_members()?.any(|m| m == id2) },
                "a relay member has cgka access before being promoted"
            );
            hive.add_member(id2, group_id, Access::Read, &[]).await?;
            assert!(
                { doc.lock().await.cgka_members()?.any(|m| m == id2) },
        ...
        //    is added to the group
        {
            ...
            let doc = hive.generate_doc(vec![], nonempty![[1u8; 32]]).await?;
            let doc_id = DocumentId(doc.lock().await.id());
            hive.add_member(group_id, doc_id, Access::Relay, &[])
                .await?;
            hive.add_member(id, group_id, Access::Read, &[]).await?;
            assert!(
                !{ doc.lock().await.cgka_members()?.any(|m| m == id) },
    ...
    async fn test_group_member_below_read_gets_no_key() -> TestResult {
        ...
        let group = hive.generate_group(vec![]).await?;
        let group_id = { group.lock().await.group_id() };
        hive.add_member(carol_id, group_id, Access::Relay, &[])
            .await?;
