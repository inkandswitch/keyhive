    ...
    > Keyhive<F, S, K, T, P, C, L, R>
{
    ...
    pub async fn revoke_member(
        ...
        let revoked_individual_ids: HashSet<IndividualId> = match resource {
            Membered::Group(_, group) => {
                let delegates: Vec<Agent<F, S, K, T, L>> = {
                    let locked = group.lock().await;
                    locked
                        .members()
                        .get(&to_revoke)
    ...
    pub async fn events_for_agent(
        &self,
        agent: &Agent<F, S, K, T, L>,
    ) -> HashMap<Digest<Event<F, S, K, T, L>>, Event<F, S, K, T, L>> {
    ...
    pub async fn try_from_archive(
        ...
        async fn reify_ops<
            ...
            J: SecretKeyStore<G>,
            U: ContentRef,
            M: MembershipListener<G, Z, J, U>,
        >(
            group: &mut Group<G, Z, J, U, M>,
            dlg_store: Arc<Mutex<DelegationStore<G, Z, J, U, M>>>,
            rev_store: Arc<Mutex<RevocationStore<G, Z, J, U, M>>>,
            dlg_head_hashes: &HashSet<Digest<Signed<StaticDelegation<U>>>>,
            rev_head_hashes: &HashSet<Digest<Signed<StaticRevocation<U>>>>,
            members: HashMap<Identifier, NonEmpty<Digest<Signed<Delegation<G, Z, J, U, M>>>>>,
        ) -> Result<(), TryFromArchiveError<G, Z, J, U, M>> {
            let read_dlgs = dlg_store.lock().await;
            let read_revs = rev_store.lock().await;

            for dlg_hash in dlg_head_hashes.iter() {
                let actual_dlg: Arc<Signed<Delegation<G, Z, J, U, M>>> = read_dlgs
                    .get(&dlg_hash.coerce())
    ...
    async fn test_async_transaction() -> TestResult {
        ...
        let tx = transact_async(
            ...
            |fork: Keyhive<
                ...
                _,
                _,
                Log<Sendable, _, MemorySecretKeyStore, [u8; 32]>,
                _,
            >| async move {
