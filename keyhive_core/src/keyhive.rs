    ...
    > Keyhive<S, T, P, C, L, R>
{
    ...
    pub async fn events_for_agent(
        ...
        let membership_ops = self.membership_ops_for_agent(agent).await;
        tracing::info!(
            "events_for_agent: Got {} membership operations",
            membership_ops.len()
        );
        ...
        let reachable_prekey_ops = self.reachable_prekey_ops_for_agent(agent).await;
        let mut prekey_count = 0_u64;
        ...
        tracing::info!("events_for_agent: Added {} prekey operations", prekey_count);

        let cgka_ops = self.cgka_ops_reachable_by_agent(agent).await?;
        let cgka_count = cgka_ops.len();
        for cgka_op in cgka_ops.into_iter() {
            let op = Event::<S, T, L>::from(cgka_op);
            ops.insert(Digest::hash(&op), op);
        }
        tracing::info!("events_for_agent: Added {} CGKA operations", cgka_count);
        tracing::info!("events_for_agent: Returning {} total events", ops.len());
