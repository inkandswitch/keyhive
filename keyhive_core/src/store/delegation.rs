...
// Methods that don't require Serialize
impl<K: FutureForm + ?Sized, S: AsyncSigner<K>, T: ContentRef, L: MembershipListener<K, S, T>> DelegationStore<K, S, T, L> {
    ...
    pub fn remove_by_hash(
        &mut self,
        hash: &Digest<Signed<Delegation<K, S, T, L>>>,
    ) -> Option<Arc<Signed<Delegation<K, S, T, L>>>> {
    ...
    pub fn values(
        ...
        '_,
        Digest<Signed<Delegation<K, S, T, L>>>,
        Arc<Signed<Delegation<K, S, T, L>>>,
    > {
...
// Methods that require Serialize (insert, contains_value)
// These have the Serialize bound on the method itself
impl<K: FutureForm + ?Sized, S: AsyncSigner<K>, T: ContentRef, L: MembershipListener<K, S, T>> DelegationStore<K, S, T, L> {
    /// Check if a [`Delegation`] is present in the store.
    pub fn contains_value(&self, value: &Signed<Delegation<K, S, T, L>>) -> bool 
    where
...
// More methods that don't require Serialize
impl<K: FutureForm + ?Sized, S: AsyncSigner<K>, T: ContentRef, L: MembershipListener<K, S, T>> DelegationStore<K, S, T, L> {
...
// from_iter_direct needs Serialize because it calls insert
impl<K: FutureForm + ?Sized, S: AsyncSigner<K>, T: ContentRef, L: MembershipListener<K, S, T>> DelegationStore<K, S, T, L> {
    /// Create a [`DelegationStore`] from an iterator of [`Delegation`]s.
    pub fn from_iter_direct(
        iter: impl IntoIterator<Item = Arc<Signed<Delegation<K, S, T, L>>>>,
    ) -> Self 
    where
        Signed<Delegation<K, S, T, L>>: Serialize,
    {
