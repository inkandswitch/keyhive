    ...
    > Document<F, S, K, T, L>
{
    ...
    pub(crate) async fn add_cgka_member(
        &mut self,
        delegation: &Signed<Delegation<F, S, K, T, L>>,
        signer: &S,
    ...
    pub async fn try_causal_decrypt_content<
        ...
        P: for<'de> Deserialize<'de> + Serialize + Clone + Eq + std::hash::Hash,
    >(
        &mut self,
        encrypted_content: &EncryptedContent<P, T>,
        store: C,
...
impl<
        ...
        T: ContentRef,
        L: MembershipListener<F, S, K, T>,
    > From<MissingDependency<Digest<Signed<Delegation<F, S, K, T, L>>>>>
    for TryFromDocumentArchiveError<F, S, K, T, L>
{
    fn from(e: MissingDependency<Digest<Signed<Delegation<F, S, K, T, L>>>>) -> Self {
