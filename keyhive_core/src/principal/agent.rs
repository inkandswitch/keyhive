...
pub enum Agent<S: Verifiable, T: ContentRef = [u8; 32], L = NoListener> {
...
impl<S: Verifiable, T: ContentRef, L> PartialEq for Agent<S, T, L> {
...
impl<S: Verifiable, T: ContentRef, L> Agent<S, T, L> {
...
impl<S: Verifiable, T: ContentRef, L> Verifiable for Agent<S, T, L> {
    fn verifying_key(&self) -> VerifyingKey {
        self.id().verifying_key()
    }
}

impl<S: Verifiable, T: ContentRef, L> Dupe for Agent<S, T, L> {
    fn dupe(&self) -> Self {
        self.clone()
    }
}

impl<S: Verifiable, T: ContentRef, L> Display for Agent<S, T, L> {
