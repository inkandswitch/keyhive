...
pub enum Peer<S: Verifiable, T: ContentRef = [u8; 32], L = NoListener> {
...
impl<S: Verifiable, T: ContentRef, L> Peer<S, T, L> {
...
impl<S: Verifiable, T: ContentRef, L> Dupe for Peer<S, T, L> {
    fn dupe(&self) -> Self {
        self.clone()
    }
}

impl<S: Verifiable, T: ContentRef, L> Clone for Peer<S, T, L> {
    fn clone(&self) -> Self {
        match self {
...
impl<S: Verifiable, T: ContentRef, L> From<Peer<S, T, L>> for Agent<S, T, L> {
...
impl<S: Verifiable, T: ContentRef, L> TryFrom<Agent<S, T, L>> for Peer<S, T, L> {
