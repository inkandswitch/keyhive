...
impl<S: AsyncSigner, T: ContentRef, L: MembershipListener<S, T>> MembershipOperation<S, T, L> {
    ...
    pub fn reverse_topsort(
        ...
        while let Some(op) = explore.pop() {
            ...
            if let MembershipOperation::Revocation(r) = &op {
                revoked_dependencies
                    .insert((*r.payload.revoke.signature()).into(), digest);
            }
        ...
        let mut longest_paths: HashMap<
            Digest<MembershipOperation<S, T, L>>,
            usize,
        > = HashMap::new();
        let mut first_pass: Vec<Vec<(
            Digest<MembershipOperation<S, T, L>>,
            &MembershipOperation<S, T, L>,
        )>> = vec![];
        ...
        for level in &first_pass {
            ...
            let revocations: Vec<_> = level
                .iter()
                .filter(|(_, op)| op.is_revocation())
                .collect();
            ...
            if revocations.len() > 1 {
                let mut sorted_revs: Vec<_> = revocations
                    ...
                    .collect();
                sorted_revs.sort_by(|(d1, _, lp1), (d2, _, lp2)| {
