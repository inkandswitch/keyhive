...
impl<S: AsyncSigner, T: ContentRef, L: MembershipListener<S, T>> MembershipOperation<S, T, L> {
    ...
    pub fn memoized_digest(&self) -> Digest<MembershipOperation<S, T, L>> {
        match self {
            MembershipOperation::Delegation(delegation) => delegation.memoized_digest().into(),
            MembershipOperation::Revocation(revocation) => revocation.memoized_digest().into(),
        }
    }
    ...
    pub fn reverse_topsort(
        ...
        let mut history: Vec<(
            ...
            .values()
            .map(|op| (op.memoized_digest(), op.clone()))
            .collect();
