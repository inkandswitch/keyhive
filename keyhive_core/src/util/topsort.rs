...
mod tests {
    ...
    fn wide_fan_out() {
        ...
        assert_eq!(ts.pop_all(), vec![0]);

        let mut second = ts.pop_all();
        second.sort();
    ...
    fn wide_fan_in() {
        ...
        let mut first = ts.pop_all();
        first.sort();
        assert_eq!(first, (0..100).collect::<Vec<_>>());

        assert_eq!(ts.pop_all(), vec![100]);
        assert!(ts.is_empty());
    }
    ...
    proptest! {
        ...
        fn prop_mid_drain_edge_respected(
            ...
            // Drain one batch
            let first_batch: BTreeSet<usize> = ts.pop_all().into_iter().collect();
