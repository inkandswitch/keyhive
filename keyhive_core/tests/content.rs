...
#[tokio::test]
async fn a_member_added_after_a_write_reads_it_from_its_invitation() -> Result<()> {
    let mut ctx = TestContext::new().await;
    let alice = ctx.individual("alice").await?;
    let bob = ctx.individual("bob").await?;
    let design_doc = ctx.doc(&alice, "design_doc").await?;
    ...
    assert_eq!(
        bob.try_decrypt_content(design_doc, &ct).await?,
        b"before bob".to_vec(),
        "and so can a member added afterwards, from the secrets its invitation wraps"
    );
    Ok(())
}
...
#[tokio::test]
async fn a_rotation_does_not_cut_off_the_secret_an_invitation_wraps() -> Result<()> {
    let mut ctx = TestContext::new().await;
    let alice = ctx.individual("alice").await?;
    let bob = ctx.individual("bob").await?;
    let design_doc = ctx.doc(&alice, "design_doc").await?;
    ...
    assert!(
        ...
        "and the rotation between them does not cut off the secret his invitation wraps"
    );
    Ok(())
}
