...
> [!NOTE]
> Audience: an agent (or human) implementing, analyzing, or discussing a Keyline evaluator. This document explains _how evaluation works and why it is shaped the way it is_ — in particular why it cannot be a single SQL query, where the genuinely hard part lives, and which intuitions are traps. It is a companion to the normative design docs, not a replacement for them.
...
Where this document and `implementation.md` disagree, `implementation.md` wins. The SQLite and PostgreSQL prototypes referred to below were built alongside this note to validate the shape of the pipeline; they are not part of the repository.
...
2. _Stencil_ — from that, compute per-certificate forbidden nodes (`covered`).
3. _Replay_ — recompute standing, filtering routes through the stencil.

The pipeline (steps 1 → 2 → 3) is trivially expressible in SQL as chained CTEs, and the negation is a legal anti-join. The hard part is _inside_ steps 1 and 3: deriving standing is a **non-linear** fixpoint (each derivation step consumes _two_ recursively derived facts), which exceeds `WITH RECURSIVE` in SQLite/PostgreSQL (both allow exactly one reference to the in-progress relation). Consequence: **one ordinary SQL statement per fixpoint round, plus a driver loop ("repeat until no new rows")**. The loop is the only non-declarative ingredient in the entire design.
...
The single most important framing: **there are two overlaid graphs, and only one of them is stored.**
...
| Power overlay | Who actually holds authority over what | _Derived._ Recomputed from the message graph at every evaluation; stored nowhere |

```mermaid
flowchart LR
    classDef socket fill:#f9f,stroke:#333

    Carol((Carol)):::socket

    Carol -. "root cert [sub: Carol]" .-> Alice((Alice))
    Alice -. "cert [sub: Carol]" .-> Bob((Bob))
    Dan((Dan)) -. "cert [sub: Carol]" .-> Bob

    Carol ==>|power| Alice
    Alice ==>|power| Bob
```

Dashed = message layer (three signed certs). Thick = power overlay (what the evaluator derives). Note the asymmetry: Dan's certificate is well-signed and sits in the set forever, but no power edge materializes over it, because no power ever reached Dan. It is _inert_ — not invalid, not an error.
...
Everything the design calls _late binding_, _healing_, _implicit death_, and _resurrection_ is an overlay phenomenon over an immutable message layer:
...
- Cutting a load-bearing cert makes downstream power edges vanish on the next evaluation — no certificate changed, only the overlay.
- Re-adding a booted key re-materializes every overlay edge its old certs supported ("zombies," see the §8 ledger for the design-vs-evaluation zombie distinction, and §7 for their cost implications).

## 2. The Power Overlay is an AND/OR Graph

The overlay is not a plain graph: it alternates between two node kinds with different combination semantics. Principals are **OR-nodes**; certificates are **AND-nodes**.
...
flowchart LR
    ...
    Members ==>|"feed 1: club's standing over Doc"| sponsor
    Alice ==>|"feed 2: issuer's standing in club"| sponsor
    sponsor ==> Carol
```

The AND-ness is where all the difficulty lives. A certificate whose `sub` is the evaluated subject itself has one feed (its issuer's standing) — those chains are ropes. A certificate whose `sub` is a _role_ ("club") has **two feeds of different flavors**:

1. the club's standing over the evaluated subject (`R(subject, club)`), and
2. the issuer's standing within the club (`R(club, issuer)`).
...
- the fixpoint → "extend the overlay outward from the socket until stable"

Useful corollaries of the AND/OR view:

- _Dead certs are visible at a glance:_ a box with a solid (message) edge but no incoming overlay feed is inert. Ungrounded islands (a club nobody ever supplied) render as overlay fragments that never reach any socket.
- _Griefing is a min-cut question on this graph:_ a grantee survives iff some AND/OR derivation avoids every cut node; redundant routes defend only when jurisdictionally disjoint (distinct OR-branches through distinct clubs).
- _Cycles resolve to dead:_ the overlay is the **least** fixpoint. A clique of certs vouching for each other with no path to a socket derives nothing. (Assume-dead-on-revisit; the greatest fixpoint would make ungrounded cycles self-certifying — a one-line bug with a security consequence.)

### Worked Sketch: Five Certificates (and a Cycle)

A minimal set exercising every mechanism. One socket (Carol), one club (Members), and a sponsorship into the club:

| # | Certificate | Kind |
|---|-------------|------|
| 1 | `{iss: Carol, aud: Alice, sub: Carol}` | root (socket self-grounds) |
| 2 | `{iss: Alice, aud: Members, sub: Carol}` | supply (club receives Carol-standing) |
| 3 | `{iss: Members, aud: Alice, sub: Members}` | roster (club self-grounds its membership) |
| 4 | `{iss: Alice, aud: Bob, sub: Members}` | sponsorship — the club-rule AND-node |
| 5 | `{iss: Bob, aud: Dan, sub: Carol}` | direct grant riding derived standing |
...
flowchart LR
    ...
    Alice ==> c2 ==> Members
    Members ==> c3 ==> Alice
    Members ==>|"feed 1: club's<br/>Carol-standing"| c4
...
| Round | New facts | Via |
|-------|-----------|-----|
| 0 | `R(Carol, Carol)`, `R(Members, Members)` | sockets |
| 1 | `R(Carol, Alice)`, `R(Members, Alice)` | #1; #3 |
| 2 | `R(Carol, Members)`, `R(Members, Bob)` | #2; #4 (roster side only) |
| 3 | `R(Carol, Bob)` | #4 (both feeds: `R(Carol, Members)` ∧ `R(Members, Alice)`) |
| 4 | `R(Carol, Dan)` | #5 |
| 5 | — (stable) | |

Lessons packed into five certs:

- _One cert, two flavors._ #4 delivers `R(Members, Bob)` (round 2, roster feed alone suffices when the evaluated subject _is_ the club) and `R(Carol, Bob)` (round 3, both feeds required). Membership conveys the club's entire present and future portfolio: supply the club with a second socket later and Bob inherits it with no new certificate — `sub` is a scope, not an endpoint.
- _The cycle is benign._ #2 and #3 form `Alice → Members → Alice`. The club route back into Alice grounds through Alice's own root standing, so it is min-clamped to what she already had: cycles amplify nothing; only sockets ground. (Least fixpoint; assume-dead-on-revisit.)
- _False redundancy._ Alice has OR fan-in (direct root + around the club), but the club route transits her own root cert — cut #1 and every fact from round 1 downward dies. Redundant routes defend only when jurisdictionally disjoint; a loop through your own standing is maximally non-disjoint. Real redundancy here requires an _independent_ supply into Members from a second Carol-standing holder.
- _Variant:_ change #5 to `sub: Members` and Dan joins the club instead: two feeds, lands round 3, inherits future club acquisitions. The choice between "grant a thing" and "grant membership" is one field.
...
|------------|------|--------|
| `rA = {iss: Bob, revoke: #2}` | third party, `admin_reach(Bob) = {Bob}` | inert: #2's routes are `{Carol, Alice}`; valid, admissible, zero effect |
| `rB = {iss: Alice, revoke: #5}` | deep cut, `admin_reach(Alice) = {Alice, Carol, Members}` | #5 dead (its every route transits the reach); #4 and Bob untouched — scoped, no cascade |
...
- _The socket can be inside an admin reach._ `#1` is a live-key `{sub: Carol, can: Admin}` cert, so `Carol ∈ admin_reach(Alice)` — and since every derivation grounds at the socket, Alice can cover _any_ cert in this graph, including #1 itself: ownership-equivalent kill power. Whether a document's root edge is deniable is not a rule; it is decided by whether any surviving key holds Admin over the socket, which the ceremony chooses by the level of the root edge ([patterns, Rooting Level](patterns.md#rooting-level)). Same rules, different certificate set, opposite outcome.
- _Reach is composed._ `admin_reach(K) = {K} ∪ {n : R⁺(n, K) = Admin}` — a row lookup over the positive pass, so Admin standing inherited through a club counts. The stricter alternative (only Admin whose final hop is a `sub: n` certificate) was considered and rejected; see [alternatives, Direct (last-hop) admin reach](alternatives.md#direct-last-hop-admin-reach).
...
```prolog
% grounding: root edges, iss = sub
R(S, Aud, Can) :- edge(S, Aud, S, Can).

% the club rule — TWO recursive premises (non-linear)
R(S, Aud, min(L1, L2, Can)) :-
    edge(Iss, Aud, Club, Can),
    R(S, Club, L1),        % feed 1: club has standing over S
    R(Club, Iss, L2).      % feed 2: issuer has standing in club
```
...
Stratum 1b (stencil):       admin_reach(K) = {n : R⁺ ⊢ K ever held Admin over n} ∪ {K}
...
- _Denials are mutually invisible._ Revocations name delegations only (`revoke: Hash<Delegation>` — a revocation of a revocation is unwritable), so no revocation's effect depends on another's. This is what makes the stencil computable in one stroke, order-independently.
...
## 4. Complexity: Why This Is Genuinely Hard (and For Whom)

| Fragment | Proof shape | Complexity | SQL analogue |
|----------|-------------|------------|--------------|
| Linear Datalog | paths ("ropes") | ⊆ NL | `WITH RECURSIVE` (single self-reference) |
| Full/non-linear Datalog | trees | P-complete | — none — |

- The club rule is non-linear (two recursive premises). This is RT₀/SDSI credential chain discovery, which Jha & Reps showed equivalent to pushdown system reachability — P-complete. The hardness predates Keyline by decades and is the price of role indirection itself, in any evaluator.
...
- A general rewrite of the club rule into linear form would place a P-complete problem in NL, i.e., prove NL = P. Some non-linear programs are linearizable (same-generation, famously); pushdown-hard ones are believed not to be.
...
> [!IMPORTANT]
> The hardness lives entirely in the **positive pass** (delegation semantics, `sub`-as-scope). It exists in a Keyline with zero revocations. No redesign of revocations — e.g. the rejected `sub`-on-`Revocation` field (alternatives.md, "A `sub` (jurisdiction) field on `Revocation`") — touches it. Conversely, deleting `sub`-as-scope from delegations would linearize everything and gut the design (no roles, no late-bound membership).
...
| The route-search fixpoint itself (S1, S2) | **No** — non-linear recursion |
...
A plain query over a results table may join two derived facts freely; the one-reference rule binds only _inside_ `WITH RECURSIVE`. What SQL lacks is not sequencing (chaining handles any _fixed_ number of steps) but a **data-dependent repetition count** — the word "until." `WITH RECURSIVE` is SQL's only built-in "until," and it walks ropes.

Escape hatches, in decreasing order of dignity:
...
2. _Bounded unrolling._ If role-nesting depth is capped at k (a policy decision — the semantics promise unbounded depth), unroll into k chained linear CTEs and the whole evaluation is genuinely one statement.
3. _State-blob trick (stunt only)._ Linear recursion where each row carries the _entire fact set_ as an array/jsonb value; the two premises come from `unnest` of a column value, not a second CTE reference, so the grammar is satisfied. Works (this is the folklore that makes recursive CTEs Turing-complete); costs O(rounds × facts) copying, defeats indexes and semi-naive deltas, and is write-only code. Acceptable as a differential test oracle, wrong as an evaluator.
...
|----------|------------|-----|
| "Is Bob reachable from Carol over _given_ edges?" | NL — transitive closure | one `WITH RECURSIVE`; SQL's home turf |
| "Is Bob reachable from Carol, where each edge only _counts_ if two other reachability facts already hold?" (Keyline) | P-complete | exceeds `WITH RECURSIVE`; needs the loop |
...
The SQLite prototype implements the full pipeline and reproduces the worked sketch: S1 converges in 5 rounds with the exact hand-derived facts, `admin_reach(Alice) = {Alice, Carol, Members}`, the inert revocation does nothing, the deep cuts kill only their targets, the retraction variant cascades implicitly, and exclusion contexts dedup (the `#5`/`#6` cuts by one revoker share a single context — see §7).
...
Stratum 1 — seed sockets once, then one statement per round (the threshold encoding: a `(lvl, s, n)` fact means "standing ≥ lvl"; no aggregates needed):
...
```sql
CREATE VIEW admin_reach AS      -- {K} itself arrives free via socket facts
...
Stratum 2 — same fixpoint over `live(ctx, lvl, s, n)`, where `ctx` is an exclusion context: `''` (unconstrained) plus **one per distinct exclusion set** — contexts are keyed by a canonical signature of the excluded-node set (`GROUP_CONCAT(node ORDER BY node)` per covered cert, deduped), _not_ per covered cert. Certs covered by the same revoker(s) share a context; this is the k³ → k² collapse of §7 and is mandatory, not an optimization nicety. The encoding implements NODES without materializing paths: a derivation in context X never transits an excluded node because the socket seed and every CONDUCT step are filtered by `excl(X, ·)`; and every hop must additionally be justifiable in its _own_ context (its own exclusion-set signature), **rooted at the hop's own subject** — `live(own, lvl, d.sub, d.iss)`, not `live(own, lvl, s, …)`. A hop about `Members` is judged inside `Members`' graph regardless of which subject is being queried; how `s` reaches `Members` is irrelevant to whether the hop conducts (the daisy-chain rule in [implementation, Evaluation](implementation.md#evaluation)). The shape is the S1 round plus three anti-joins (`NOT EXISTS` against `covered`/`excl`), which are legal everywhere, plus one feed-join for the hop's own-context check. Fact blowup is |contexts| = 1 + |distinct revoker-sets among covered certs| — tighter than the spec's per-dispute cost model. Final answer: `SELECT s, n, MAX(lvl) FROM live WHERE ctx = '' GROUP BY s, n`.
...
The PostgreSQL prototype is the entire pipeline as one PL/pgSQL function (temp tables per stratum, `LOOP ... GET DIAGNOSTICS ... EXIT WHEN 0` around each round statement), validated against the worked-example scenarios (deep cut; retraction cascade). No external driver of any kind — callers just write:
...
```prolog
% stratum 1 — positive pass; two IDB atoms = the club rule, no ceremony
...
Note what Datalog does _not_ buy: evaluation is still P-complete — the engine's semi-naive evaluator contains exactly the "until stable" loop the SQL driver hand-writes. The lfp is a language primitive instead of an external driver; the rounds and deltas are identical. Datalog moves the loop, it does not delete it.

### Incremental Evaluation: DBSP

DBSP (the Z-set/stream-circuit theory under Feldera; Budiu, McSherry et al.) is arguably the theoretically correct engine for this workload. Z-sets are multisets with signed multiplicities — _retractions are first-class_ — and any query circuit, including recursive fixpoints and stratified negation, can be mechanically differentiated into an incremental circuit that processes input deltas in time proportional to change size. Its party trick is nested time: fixpoint iteration (inner clock) and input evolution (outer clock) as two stream dimensions, incrementalized independently. A satisfying theoretical bonus: semi-naive evaluation falls out as _literally the derivative of naive evaluation_ — the hand-written loops in the SQL prototypes and `MemoryKeyline` are the inner-clock derivative circuit, derived manually.

What the outer clock buys, against the bottom-up evaluators (`MemoryKeyline`, the SQL prototypes):
...
Deployment reality: `keyline` is `no_std`/Wasm-postured; the `dbsp` crate is a std, multithreaded runtime. The honest architecture is two-tier — embedded replicas (apps, Wasm) run a `memory.rs`-style evaluator with digest memoization and an S1 frontier cache; heavy replicas (relays, sync servers, org indexers) run a DBSP/Feldera circuit. That places the strongest DoS defense exactly where update volume and exposure concentrate. Differential-dataflow/Materialize occupy the same niche; DBSP's edge here is the cleaner theory, an embeddable-ish Rust library, and Feldera's SQL frontend with first-class recursive views.
...
- _Cache stratum 1 aggressively._ It consults only delegations, which are append-only; admin reach and coverage grow monotonically and never retract. Merges evaluate deltas from the cached frontier. Stratum 2 is the disputed-certificate tax: uncovered certs share one pass; each covered cert pays a route search with its own exclusion mask. A jurisdiction accumulating cuts is one under dispute; rotation moots the cuts and restores the fast path.
...
Evaluation is superlinear (quadratic fact space; more under dispute), which raises the question: can an adversary weaponize the evaluator? Answer: yes, but only from _inside_ the authorization graph — and the boundary between tiers is sharp. This analysis is absent from the design docs (their griefing section prices authority-denial only, not compute-denial); treat this section as a supplement, not a summary.

### Tier 0 — Outsiders: storage spam only

Anyone can sign anything, but evaluation forward-chains from sockets: a cert whose issuer never receives standing produces _zero rule instantiations_ ("junk never enters the fixpoint" is a DoS defense). Free defenses:
...
```text
club₁ over Doc, club₂ member of club₁, …, clubₖ member of clubₖ₋₁
...
**But the k³ is an encoding artifact, not a semantic cost.** `covered(c)` depends only on _who revoked c_, and one ex-admin's frozen reach is one set: a k-cut spree by one key produces k _identical_ exclusion sets. Dedup contexts by exclusion-set signature and the spree costs **one** context — O(k²) shared work plus k cheap per-cert checks. To multiply contexts the adversary needs multiple _distinct booted admin keys_ (each legitimately granted Admin at some point, each with a different frozen reach) — a high bar with a built-in audit trail. The SQL prototypes implement the deduped encoding and assert it (`#5`/`#6` cut by one revoker → one shared context).
...
> 3. Top-down/demand-driven evaluators MUST demand the socket-side feed first and short-circuit on its failure, and SHOULD memoize failed demands per set-state. Club internals (self-grounded rosters, which an attacker shapes to be expensive) must never be explored until the club's standing over the query's subject is established. Bottom-up evaluators — `MemoryKeyline` and the SQL prototypes — are immune by construction: dead facts are never derived, so a dead ladder is never walked.
...
Keyline's k² is #3: because clubs are themselves subjects, the fact space is `(subject × node)` — materializing all standing is inherently closure-shaped, and the club ladder makes the closure genuinely contain Θ(k²) true facts. No evaluator avoids Ω(output).
...
The forcing construction — the **gift-cert attack** — is why demand-driven evaluation does not lower the worst case. Certificates are issuer-signed only; recipients never consent. So relevance is attacker-writable:

```text
1. Attacker (Tier 1 member) builds a k-club ladder with adversarially
...
| Dead-ladder exploration bait | no — evaluator artifact | obligation 3 above (socket-first ordering); note a booted attacker's ladder stays _internally_ self-grounded, which is exactly what obligation 3 defends against |
...
Traps encountered while deriving the above. Each was plausible; each is wrong in an instructive way.
...
| "Graph reachability is inherently quadratic (Dijkstra/A* are n²)" | Single-source reachability is O(V + E) — linear; the remembered n² is the array-based Dijkstra implementation artifact. Keyline's k² is the all-pairs/output-size bound (#3 in §7's disambiguation), which applies because clubs are subjects and the closure is the answer — not because reachability is expensive |
| "Each covered cert needs its own route search (hence k³ sprees)" | `covered(c)` depends only on who revoked c; one ex-admin's frozen reach is one set. Key contexts by exclusion-set signature and a one-key spree costs one context (§7, evaluator obligation 1). The k³ was an encoding artifact this document itself taught until corrected |
...
- The spec's worked example (README §Worked Example: Doc, Bob, Members, Alice, M2, Carol — boot, heal, re-grant, zombie) exercises every mechanism in eight certificates; encode it first.
...
### Gift-cert scenario (specified, not yet implemented)

The one §7 scenario no existing evaluator exercises — a test-case spec for whoever implements a demand-driven evaluator:

```text
certs:
  L1..Lk : a k-club ladder, self-grounded, with nested rosters
...
phases and assertions:
  ...
  3. boot:        revoke the attacker's Doc-standing. Assert the query no
                  ...
                  (obligation 3: socket-side feed first). This is the
...
|------|--------|---------------|
| Context dedup by exclusion set | Specified here (§7, obligation 1); `MemoryKeyline` searches per covered certificate | `../../keyline/src/memory.rs` `live_set` / `caps` |
| Early-exit inert disputes (obligation 2) | `MemoryKeyline` prunes covered edges whose issuer is unreachable in the shared pass | §7 IMPORTANT callout |
| Socket-first demand ordering (obligation 3) | Moot for bottom-up evaluators; binding for any future demand-driven one | §7; gift-cert scenario (§9) |
| Gift-cert conformance scenario | Specified (§9), not implemented | §9; `../../keyline/src/conformance/scenarios.rs` |
| Compute-denial analysis in the model document | §7 has no counterpart in `README.md` (griefing there prices authority-denial only) | §7; `README.md` Cost / Griefing |
| Depth cap as a semantic lever | Analyzed, not recommended; consensus-critical if ever adopted (all replicas must agree) | §7 mitigation checklist, closing paragraph |
| Differential testing of SQL backends | The prototypes were validated against the worked example only; `conformance::gen::CertSet` can drive them against `MemoryKeyline` | `../../keyline/src/conformance/gen.rs` |

## Glossary

| Term | Meaning |
|------|---------|
| AND-node | A certificate in the overlay: conducts iff all feeds live; output = min of feeds and own `can` |
...
| Club rule | The non-linear derivation step: a `sub`-scoped cert needs the club's standing over the subject AND the issuer's standing in the club |
| Covered / stencil | The per-certificate `(cert, forbidden node)` relation cut from the positive pass; applied as an anti-join during replay |
...
| OR-node | A principal in the overlay: standing = max over incident conducting certs |
| Overlay / positive graph | Derived standing facts; recomputed per evaluation; stored nowhere |
| Rope vs tree | Linear vs non-linear proof shape; the boundary between `WITH RECURSIVE` and a driver loop |
| Socket | The subject grounding an evaluation (root edges: `iss = sub`) |
