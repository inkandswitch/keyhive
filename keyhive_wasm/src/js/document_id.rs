...
impl JsDocumentId {
    ...
    pub fn new(bytes: Vec<u8>) -> Result<Self, JsError> {
        let vec: [u8; 32] = bytes.try_into().map_err(|_| JsError::CannotParseIdentifier)?;

        // NOTE signature::Error is opaque, so we can just ignore the inbuilt error
        let vk =
            ed25519_dalek::VerifyingKey::from_bytes(&vec).map_err(|_| JsError::CannotParseIdentifier)?;
