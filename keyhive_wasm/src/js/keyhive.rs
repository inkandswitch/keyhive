...
impl JsKeyhive {
    ...
    pub async fn generate_group(&self, coparents: &[JsPeer]) -> Result<JsGroup, JsSigningError> {
        let group = self
            .0
    ...
    pub async fn try_encrypt_archive(
        &self,
        doc: &JsDocument, // FIXME make all incominh values borrows
        content_ref: JsChangeRef,
