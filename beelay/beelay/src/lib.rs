...
use beelay_core::{
    ...
    Audience, BundleSpec, CommandResult, PeerAddress, PeerId, RequestId, RpcResponse,
    SignedMessage, SnapshotId, UnixTimestamp,
};
pub use beelay_core::{
...
>(
    ...
    let mut awaiting_commands = HashMap::new();
    ...
    while let Some(event) = events.next().await {
        let event_results = match event {
            Event::Input(message) => match message {
                ...
                Message::NewStream(direction, forwarding, reply) => {
                    ...
                    awaiting_commands.insert(command_id, AwaitingCommand::CreateStream(reply));
                    core.handle_event(UnixTimestamp::now(), event)
                }
                Message::DisconnectStream(stream_id, reply) => {
                    let (command_id, event) = beelay_core::Event::disconnect_stream(stream_id);
                    awaiting_commands.insert(command_id, AwaitingCommand::DisconnectStream(reply));
                    core.handle_event(UnixTimestamp::now(), event)
                }
                Message::HandleMessage(stream, msg, reply) => {
                    let (command_id, event) = beelay_core::Event::handle_message(stream, msg);
                    awaiting_commands.insert(command_id, AwaitingCommand::HandleMessage(reply));
                    core.handle_event(UnixTimestamp::now(), event)
                }
                Message::RegisterEndpoint(remote_audience, forwarding, reply) => {
                    let (command_id, event) =
                        beelay_core::Event::register_endpoint(remote_audience, forwarding);
                    awaiting_commands.insert(command_id, AwaitingCommand::RegisterEndpoint(reply));
                    core.handle_event(UnixTimestamp::now(), event)
                }
                Message::UnregisterEndpoint(endpoint_id, reply) => {
                    let (command_id, event) = beelay_core::Event::unregister_endpoint(endpoint_id);
                    awaiting_commands
                        .insert(command_id, AwaitingCommand::UnregisterEndpoint(reply));
                    core.handle_event(UnixTimestamp::now(), event)
                }
                ...
                Message::CreateDoc(reply) => {
                    let (command_id, event) = beelay_core::Event::create_doc();
                    awaiting_commands.insert(command_id, AwaitingCommand::CreateDoc(reply));
                    core.handle_event(UnixTimestamp::now(), event)
                }
                Message::AddCommits(reply, dag_id, commits) => {
                    let (command_id, event) = beelay_core::Event::add_commits(dag_id, commits);
                    awaiting_commands.insert(command_id, AwaitingCommand::AddCommits(reply));
                    core.handle_event(UnixTimestamp::now(), event)
                }
                Message::SyncDoc(reply, doc_id, peer) => {
                    let (command_id, event) = beelay_core::Event::sync_doc(doc_id, peer);
                    awaiting_commands.insert(command_id, AwaitingCommand::SyncDoc(reply));
                    core.handle_event(UnixTimestamp::now(), event)
                }
                Message::AddLink(reply, addlink) => {
                    let (command_id, event) = beelay_core::Event::add_link(addlink);
                    awaiting_commands.insert(command_id, AwaitingCommand::AddLink(reply));
                    core.handle_event(UnixTimestamp::now(), event)
                }
                Message::AddBundle(reply, doc, bundle) => {
                    let (command_id, event) = beelay_core::Event::add_bundle(doc, bundle);
                    awaiting_commands.insert(command_id, AwaitingCommand::AddBundle(reply));
                    core.handle_event(UnixTimestamp::now(), event)
                }
                Message::LoadDoc(reply, dag_id) => {
                    let (command_id, event) = beelay_core::Event::load_doc(dag_id);
                    awaiting_commands.insert(command_id, AwaitingCommand::LoadDoc(reply));
                    core.handle_event(UnixTimestamp::now(), event)
                }
                ...
                Message::Listen(peer, snapshot, reply) => {
                    let (command_id, event) = beelay_core::Event::listen(peer, snapshot);
                    awaiting_commands.insert(command_id, AwaitingCommand::Listen(reply));
                    core.handle_event(UnixTimestamp::now(), event)
                }
            },
        ...
        for (command_id, result) in event_results.completed_commands {
            let Ok(result) = result else {
                // We're stopping, just drop the result
                continue;
            };
            let awaiting = awaiting_commands.remove(&command_id).unwrap();
            match (result, awaiting) {
                (
                    CommandResult::RegisterEndpoint(endpoint_id),
                    AwaitingCommand::RegisterEndpoint(reply),
                ) => {
                ...
                (CommandResult::UnregisterEndpoint, AwaitingCommand::UnregisterEndpoint(reply)) => {
                    if let Err(e) = reply.send(()) {
                        tracing::warn!(err=?e, "error sending unregister_endpoint reply");
                    }
                }
                (CommandResult::CreateStream(stream_id), AwaitingCommand::CreateStream(reply)) => {
                    let (tx, rx) = mpsc::channel(16);
                ...
                (CommandResult::DisconnectStream, AwaitingCommand::DisconnectStream(reply)) => {
                    if let Err(e) = reply.send(()) {
                        tracing::warn!(err=?e, "error sending disconnect_stream reply");
                    }
                }
                (CommandResult::HandleMessage(result), AwaitingCommand::HandleMessage(reply)) => {
                    if let Err(e) = reply.send(result) {
                        tracing::warn!(err=?e, "error sending handle_message reply");
                    }
                }
                (CommandResult::CreateDoc(doc_id), AwaitingCommand::CreateDoc(reply)) => {
                    if let Err(e) = reply.send(doc_id) {
                        tracing::warn!(err=?e, "error sending create_dag reply");
                    }
                }
                (
                    CommandResult::AddCommits(new_required_bundles),
                    AwaitingCommand::AddCommits(reply),
                ) => {
                    if let Err(e) = reply.send(new_required_bundles) {
                        tracing::warn!(err=?e, "error sending add_commits reply");
                    }
                }
                (CommandResult::SyncDoc(result), AwaitingCommand::SyncDoc(reply)) => {
                    if reply.send(result).is_err() {
                        tracing::warn!("error sending sync_doc reply");
                    }
                }
                (CommandResult::AddLink, AwaitingCommand::AddLink(reply)) => {
                    if let Err(e) = reply.send(()) {
                        tracing::warn!(err=?e, "error sending add_link reply");
                    }
                }
                (CommandResult::LoadDoc(dag), AwaitingCommand::LoadDoc(reply)) => {
                    if let Err(e) = reply.send(dag) {
                        tracing::warn!(err=?e, "error sending load_dag reply");
                    }
                }
                (CommandResult::AddBundle, AwaitingCommand::AddBundle(reply)) => {
                    if let Err(e) = reply.send(()) {
                        tracing::warn!(err=?e, "error sending add_bundle reply");
                    }
                }
                (CommandResult::Listen(result), AwaitingCommand::Listen(reply)) => {
                    if let Err(e) = reply.send(result) {
                ...
                (result, awaiting) => {
                    panic!(
                        "unexpected result {:?} for awaiting command {:?}",
                        result, awaiting
                    );
                }
            }
