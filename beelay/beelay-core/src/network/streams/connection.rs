...
use super::{InnerRpcResponse, TaskContext};
...
impl<R: rand::Rng + rand::CryptoRng> AuthCtx for TaskContext<R> {
    ...
    where
        for<'b> T: crate::serialization::Parse<'b>,
    {
        self.auth().authenticate_received_msg(msg, receive_audience)
    }
    ...
    where
        T: crate::serialization::Encode,
    {
        self.auth().sign_message(audience, msg)
    }

    fn update_offset(&self, remote_audience: Audience, their_clock: UnixTimestamp) {
        self.auth().update_offset(remote_audience, their_clock);
    }
...
impl<'a, A: AuthCtx> AuthCtx for &'a A {
    ...
    where
        T: crate::serialization::Encode,
    {
        (*self).sign_message(audience, msg)
    }
...
impl Message {
    fn new_hello<A: AuthCtx>(ctx: &A, remote_audience: Audience) -> Message {
        let hello = ctx.sign_message(remote_audience, EncodeBytes::from("hello".as_bytes()));
        Message::Hello(hello)
    }

    fn new_hello_back<A: AuthCtx>(ctx: &A, remote_audience: Audience) -> Message {
        let hello_back =
            ctx.sign_message(remote_audience, EncodeBytes::from("hello_back".as_bytes()));
...
impl Handshake {
    ...
    pub(crate) fn connect<A: AuthCtx>(ctx: &A, remote_audience: Audience) -> Step {
        let msg = Message::new_hello(ctx, remote_audience).encode();
        Step {
    ...
    pub(crate) fn receive_message<A: AuthCtx>(
        self,
        ctx: &A,
        msg: Vec<u8>,
        ...
        match (msg, self) {
            (Message::Hello(m), Handshake::AwaitingHello { receive_audience }) => {
                ...
                let hello_back =
                    Message::new_hello_back(ctx, Audience::peer(&their_peer_id)).encode();
                Ok(Step {
            ...
            (Message::Hello(m), Handshake::AwaitingHelloBack { remote_audience }) => {
                ...
                if ctx.our_peer_id().as_key().as_bytes() < sender_key.as_bytes() {
                    let hello_back = Message::new_hello_back(ctx, remote_audience).encode();
                    Ok(Step {
                        state: Connecting::Complete(Connection {
                            last_req_id: ConnRequestId::acceptors_request_id(),
                            our_peer_id: ctx.our_peer_id(),
            ...
            (Message::HandshakeFailure(f), state) => {
                ...
                match (f, state) {
                    (
                        ...
                        let msg_bytes = Message::new_hello(&ctx, remote_audience).encode();
                        Ok(Step {
                            state: Connecting::Handshaking(Handshake::AwaitingHelloBack {
                                remote_audience,
                            }),
...
mod tests {
    ...
    impl TestCtx {
        fn new() -> Self {
            Self::new_with_clock(UnixTimestamp::now())
        }

        fn new_with_key(signing_key: SigningKey) -> Self {
            let auth = crate::auth::manager::Manager::new(signing_key);
        ...
        fn new_with_clock(clock: UnixTimestamp) -> Self {
        ...
        fn signing_key(&self) -> SigningKey {
            self.auth.borrow().signing_key.clone()
        }
    }
    ...
    impl super::AuthCtx for TestCtx {
        ...
        fn our_peer_id(&self) -> crate::PeerId {
            self.auth.borrow().signing_key.verifying_key().into()
        }
    ...
    fn successful_handshake() {
        ...
        let mut computers = TwoComputers {
            left_state: TestCtx::new(),
            right_state: TestCtx::new(),
        };
        ...
        let right = Handshake::connect(
            ...
            Audience::peer(&computers.left_state.signing_key().verifying_key().into()),
        );
    ...
    fn incorrect_connect_audience_fails() {
        ...
        let right = Handshake::connect(&computers.right_state, Audience::service_name("wrong!"));

        let e = computers.run_until_connected(left, right).unwrap_err();
        assert_eq!(
            e,
    ...
    fn clock_drift_is_corrected() {
        init_logging();
        let mut computers = TwoComputers {
            left_state: TestCtx::new_with_clock(UnixTimestamp::now() + Duration::from_secs(3600)),
            right_state: TestCtx::new_with_clock(UnixTimestamp::now()),
        };
    ...
    fn request_ids_are_correctly_assigned() {
        ...
        let right = Handshake::connect(&computers.right_state, Audience::service_name("service"));

        let Connected {
            mut left,
            mut right,
        } = computers.run_until_connected(left, right).unwrap();
    ...
    fn if_both_connect_then_connect_completes() {
        for _ in 0..10 {
            ...
            let mut computers = TwoComputers {
                left_state: TestCtx::new_with_key(left_key),
                right_state: TestCtx::new_with_key(right_key),
            };
            let left = Handshake::connect(&computers.left_state, Audience::peer(&right_peer_id));
            let right = Handshake::connect(&computers.right_state, Audience::peer(&left_peer_id));

            let Connected { left, right } = computers.run_until_connected(left, right).unwrap();
