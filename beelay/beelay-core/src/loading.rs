...
use crate::{
    ...
    Beelay, IoTaskId, StorageKey, UnixTimestamp,
};
...
///
/// ```rust
/// # use your_crate::{Loading, Step, IoResult, LoadCtx};
...
pub struct Loading<R: rand::Rng + rand::CryptoRng + Clone> {
    rng: R,
...
pub enum Step<R: rand::Rng + rand::CryptoRng + Clone> {
    Loading(Loading<R>, Vec<IoTask>),
    Loaded(Beelay<R>),
}

impl<R: rand::Rng + rand::CryptoRng + Clone + 'static> Loading<R> {
    pub fn new(rng: R, now: UnixTimestamp, signing_key: SigningKey) -> (Self, Vec<IoTask>) {
        let tasks = Rc::new(RefCell::new(Tasks::new()));
        let ctx = LoadCtx {
            tasks: tasks.clone(),
        };
        let load_fut = load_keyhive(ctx, now, signing_key.clone()).boxed_local();
